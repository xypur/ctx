...
技能发布在项目根（`skills/ctx`，镜像 `skills-zh/ctx`），整体复制
...
passed —— 三个示例目录全部 doctor-healthy。

## bug-fix

### Decision

实现期两处修正记入 tooling CHANGELOG：thread slug 校验改用独立
...
passed —— 工具注入测试 12/12（逐违约类别注入、空骨架健康
