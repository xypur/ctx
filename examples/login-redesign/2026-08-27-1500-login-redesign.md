...
**Decision**: Expired sessions hard-logged users out mid-task and the token
...
**Consequences**: Fewer forced logouts and no data loss on expiry; accepted
...
**Verification**: passed — `npm test -- auth` (unit) and
...
**Decision**: Token masking before any logging folds into the same
interceptor rather than a separate fix — one guard at the shared choke point.

**Verification**: Not run — interceptor still logs the raw header in debug
mode (see R3).
